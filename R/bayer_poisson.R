#' Poisson Regression Model Bayesian analysis
#'
#' This function compile a Poisson model and returns a model with posterior probabilities
#' based on the outcome specified in the input data.frame. The model that is fitted will be
#' compiled as a brms::brm() model of brms::poisson() family with several parameters that
                  ...
                  print = FALSE )
{
  ...
  ### convert outcome
  data2[[ outcome ]] <- brms_outcome_var( data2[[ outcome ]], positive = NULL, family = 'poisson' )
  ...
  if ( "cmdstanr" %in% base::rownames( utils::installed.packages()) ) {
    if ( check_cmdstanr_out == 0 ) {
      ...
      OUT <- utils::capture.output(model <- brms::brm(formula = FF1,
                                                      prior = priors,
                                                      data = data2,
                                                      family = brms::brmsfamily( 'poisson', 'log'),
                                                      chains = brm_chains,
                                                      cores = cores,
                                                      backend = "cmdstanr",
